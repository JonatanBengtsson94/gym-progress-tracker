...
func TestInMemorySessionRepository_CreateAndGetSession(t *testing.T) {
	repo := auth.NewInMemorySessionRepository(time.Hour)

	created := repo.CreateSession(1)

	got, err := repo.GetSessionBySessionId(created.SessionId)
	if err != nil {
...
func TestInMemorySessionRepository_GetSession_NotFound(t *testing.T) {
	...
	_, err := repo.GetSessionBySessionId(uuid.New())
...
func TestInMemorySessionRepository_GetSession_Expired(t *testing.T) {
	repo := auth.NewInMemorySessionRepository(-time.Minute)

	created := repo.CreateSession(1)

	_, err := repo.GetSessionBySessionId(created.SessionId)
	if !errors.Is(err, auth.ErrSessionNotFound) {
...
func TestInMemorySessionRepository_ConcurrentAccess(t *testing.T) {
	...
	for i := range 20 {
		...
		go func(userId uint32) {
			...
			if _, err := repo.GetSessionBySessionId(session.SessionId); err != nil {
				errs <- err
			}
		}(uint32(i))
	}
